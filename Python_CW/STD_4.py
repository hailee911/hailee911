...
class Graph():
    ...
    def primMST(self):
        key = [
                  sys.maxsize] * self.V  # initialise key to list of values all set to infinity; same length as self.V
        parent = [None] * self.V  # list for constructed MST
        key[
            0] = 0  # set first element of key to zero (this is where we start)
        ...
        for vertex in range(self.V):  # go through all vertices
            ...
            for v in range(self.V):  # go through all vertices
                if 0 < self.graph[u][v] < key[v] and mstSet[v] == False:
                    ...
                    # and mstSet[v] is unreached and key[v] is greater than the edge cost
